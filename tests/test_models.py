...
from src.ecgai_data_physionet import models
from src.ecgai_data_physionet.models import EcgRecord, EcgLeadRecord, DiagnosticCode
...
def setup_test_record_data():
    ...
    path = os.path.join("./test_data", "00001_hr.json")
...
def test_create_data_set_record(caplog):
    ...
    with caplog.at_level(level=module_logging_level(), logger=logger_name()):
        ...
        record = EcgRecord.create(record_id=1, record_name="messageId", database_name="database", sample_rate=200, leads=test_leads)
